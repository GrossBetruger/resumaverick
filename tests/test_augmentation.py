...
def test_apply_multiple_augmentations_with_ratios():
    ...
    def upper(x): return x.upper()
    def lower(x): return x.lower()
    def add_dot(x): return x + "."
    ratios = [0.5, 0.25, 1]
    new_df = aug.apply_multiple_augmentations(df=df, x_col="X", y_col="y", augmentations=[upper, lower, add_dot], ratios=ratios, verbose=False)
...
def test_sentence_shuffle_with_summary():
    ...
    assert out == "Blah blah blah. Summary: Likes to code.  Seasoned software engineer. Has a PhD in math, and many hobbies.Experience: 10 years of experience in software engineering."